...
# # def register_instance_to_target_groups(aws_region, instance_id, target_groups):
...
# #             target_groups = get_target_groups_for_node(api_key, cluster_id, node_info['config_id'])
# #             if not target_groups:
# #                 logger.warning(f"No target groups for node {node_info['name']}")
# #                 continue
            
# #             result = register_instance_to_target_groups(aws_region, node_info['instance_id'], target_groups)
...
#                         logger.info(f"Already registered to {tg_arn}")
#                 except Exception as e:
#                     # If describe_target_health fails, add the ARN to 'failed'
#                     logger.error(f"Error checking target group {tg_arn}: {e}")
#                     results['failed'].append(tg_arn)  # Add to 'failed' list if an exception occurs
#                     continue  # Continue processing the next target group

#     except Exception as e:
#         logger.error(f"Error fetching target groups: {e}")
#         # If an error occurs during pagination, assume failure for all target groups
#         results['failed'].extend([tg['arn'] for tg in target_groups]) 
